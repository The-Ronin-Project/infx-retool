...
>
  ...
  <SelectWidget
    id="dropdown2"
    label="Property"
    labels="['code', 'display', 'ecl (SNOMED only)', 'RxClass','has_fhir_terminology', 'include_entire_code_system', 'all_active_rxnorm']"
    placeholder="Select a value"
    values="['code', 'display', 'ecl', 'RxClass','has_fhir_terminology', 'include_entire_code_system', 'all_active_rxnorm']"
  />
  <SelectWidget
    id="dropdown3"
    label="Operator"
  ...
  <ButtonWidget
    id="button1"
    disabled="{{ value_set_version_metadata.data.status == 'active'}}"
    events={[
      {
        ordered: [
          { event: "click" },
